...
days = [  # (date, total_sales, orders, returns)
    (date(2026, 7, 24), 150.00, 2, 12.00),  # today (incl. $12 refund)
    (date(2026, 7, 20), 25.00, 1, 5.00),    # within 7d
    (date(2026, 7, 5), 10.00, 1, 0.00),     # within 30d only
    (date(2026, 6, 1), 999.00, 5, 40.00),   # older than 30d
]

# 1) aggregation windows (7d = today + 6 prior; 30d = today + 29 prior)
w = {x["label"]: x for x in shopify._aggregate(days, today)}
assert w["Today"]["orders"] == 2 and w["Today"]["revenue"] == 150.00 and w["Today"]["refunds"] == 12.00, w["Today"]
assert w["Last 7 days"]["orders"] == 3 and w["Last 7 days"]["revenue"] == 175.00 and w["Last 7 days"]["refunds"] == 17.00, w["Last 7 days"]
assert w["Last 30 days"]["orders"] == 4 and w["Last 30 days"]["revenue"] == 185.00, w["Last 30 days"]
print("aggregate OK:", {k: (v["orders"], v["revenue"], v["refunds"]) for k, v in w.items()})
...
cols = ["day", "total_sales", "orders", "returns"]
as_dicts = [{"day": "2026-07-24", "total_sales": "150.00", "orders": 2, "returns": "-12.00"}]
as_lists = [["2026-07-24", "150.00", 2, "-12.00"]]
assert shopify._parse_rows(cols, as_dicts) == [(date(2026, 7, 24), 150.0, 2, 12.0)], shopify._parse_rows(cols, as_dicts)
assert shopify._parse_rows(cols, as_lists) == [(date(2026, 7, 24), 150.0, 2, 12.0)], shopify._parse_rows(cols, as_lists)
# day-timestamp with time component still parses; missing 'returns' -> 0
assert shopify._parse_rows(["day", "total_sales", "orders"],
                           [{"day": "2026-07-24T00:00:00", "total_sales": "5", "orders": "1"}]) \
    == [(date(2026, 7, 24), 5.0, 1, 0.0)]
print("parse OK")

# 4) render_sales handles data + error
shopify.sales = lambda: {"shop": "USA Gundam Store", "currency": "USD",
                         "windows": [{"label": "Today", "orders": 2, "revenue": 150.0, "refunds": 12.0},
                                     {"label": "Last 7 days", "orders": 3, "revenue": 175.0, "refunds": 17.0},
                                     {"label": "Last 30 days", "orders": 4, "revenue": 185.0, "refunds": 57.0}]}
html = A.render_sales()
assert "Shopify Sales" in html and "$150.00" in html and "(USD)" in html and "2 orders" in html and "USA Gundam Store" in html
assert "Refunds today: $12.00" in html, html
print("render OK")
